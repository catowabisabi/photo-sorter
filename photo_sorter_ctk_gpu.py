...
LABEL_PROMPTS = {
    ...
    "white_dog": "a clear photo of a white dog",
    "white dog": "a clear photo of a white dog",
...
class PhotoSorterApp(ctk.CTk):
    def __init__(self):
        ...
        self.labels_var = ctk.StringVar(value="food,landscape,document,white_dog")
    ...
    def classify_image(self, path: Path, labels: list[tuple[str, str]]) -> tuple[str, str]:
        try:
            ...
            if score < CATEGORY_SCORE_THRESHOLD or gap < CATEGORY_GAP_THRESHOLD:
    ...
    def _sort_by_category(self):
        ...
        category_folders = {PERSON, NOT_CLASSIFIED, *raw_labels}
        files = list(iter_category_images(folder, category_folders))
        ...
        for path in files:
            if not path.exists():
                self.log(f"Skipped missing file: {path.name}")
                continue
            category, reason = self.classify_image(path, labels)
            try:
                dest = move_file(path, folder / category)
            except FileNotFoundError:
                self.log(f"Skipped missing file while moving: {path.name}")
                continue
            moved += 1
